...
describe("Director's Pick mode detection", () => {
  ...
  it("includes igm/technology-engineering role", () => {
    const roles = directorsPickData.roles.filter((r) => r.companySlug === "igm/technology-engineering");
    expect(roles.length).toBeGreaterThan(0);
    expect(roles[0].company).toBe("IGM Financial");
    expect(roles[0].title).toBe("Technology Engineering Intern");
  });

  it("visitor via /for/igm/technology-engineering should match IGM role", () => {
    const companyConfig = {
  ...
  it("includes td/finance-data role", () => {
    const roles = directorsPickData.roles.filter((r) => r.companySlug === "td/finance-data");
    expect(roles.length).toBeGreaterThan(0);
    expect(roles[0].company).toBe("TD Bank");
    expect(roles[0].title).toBe("Finance Data Campus Intern");
  });

  it("visitor via /for/td/finance-data should match TD role", () => {
    const companyConfig = {
  ...
  it("includes suncor/engineering-student role", () => {
    ...
    expect(roles.length).toBeGreaterThan(0);
    expect(roles[0].company).toBe("Suncor");
    expect(roles[0].title).toBe("Automation, Software or Computer Engineering Student");
  });

  it("visitor via /for/suncor/engineering-student should match Suncor Engineering Student role", () => {
    const companyConfig = {
  ...
  it("includes atco/data-analyst role", () => {
    const role = directorsPickData.roles.find((r) => r.companySlug === "atco/data-analyst");
    expect(role).toBeDefined();
    expect(role?.company).toBe("ATCO");
  });

  it("sets correct sessionStorage for atco/data-analyst", () => {
    const config = getCompanyRoleConfig("atco", "data-analyst");
    expect(config).not.toBeNull();
  ...
  it("includes gdms/software-development-test role", () => {
    ...
    expect(role).toBeDefined();
    expect(role?.company).toBe("General Dynamics Mission Systems - Canada");
  });

  it("sets correct sessionStorage for gdms/software-development-test", () => {
    const config = getCompanyRoleConfig("gdms", "software-development-test");
    expect(config).not.toBeNull();
    expect(config?.brandColor).toBe("#003087");
    expect(config?.brandAccent).toBe("#ffffff");
  });
  ...
  it("includes deloitte/wonder-analyst role", () => {
    ...
    expect(role).toBeDefined();
    expect(role?.company).toBe("Deloitte");
  });

  it("sets correct sessionStorage for deloitte/wonder-analyst", () => {
    const config = getCompanyRoleConfig("deloitte", "wonder-analyst");
    expect(config).not.toBeNull();
    expect(config?.brandColor).toBe("#80b625");
    expect(config?.brandAccent).toBe("#ffffff");
  });
  ...
  it("sets correct sessionStorage for cibc/software-developer", () => {
    const config = getCompanyRoleConfig("cibc", "software-developer");
    expect(config).not.toBeNull();
    expect(config?.brandColor).toBe("#9D2235");
    expect(config?.brandAccent).toBe("#FFD500");
  });

  it("includes cibc/tech-operations-developer role", () => {
    const role = directorsPickData.roles.find((r) => r.companySlug === "cibc/tech-operations-developer");
    expect(role).toBeDefined();
    expect(role?.company).toBe("CIBC");
  });

  it("sets correct sessionStorage for cibc/tech-operations-developer", () => {
    const config = getCompanyRoleConfig("cibc", "tech-operations-developer");
    expect(config).not.toBeNull();
    expect(config?.brandColor).toBe("#9D2235");
    expect(config?.brandAccent).toBe("#FFD500");
  });
